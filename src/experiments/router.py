"""Router-Experiment: Multi-Modell LLM-as-a-Router mit Scheduler.

Das Router-Modell (Qwen3.5-2B) klassifiziert Prompts, wählt ein
Zielmodell und entscheidet über Thinking/Reasoning. Der Scheduler
verwaltet den VRAM dynamisch (Laden/Offloaden/Entladen von Modellen).

Ablauf pro Batch:
    1. Router klassifiziert alle Prompts (Modell + Thinking-Entscheidung)
    2. Execution Plan: Reihenfolge optimiert (RUNNING > SLEEPING > STOPPED)
    3. Pro Modell-Gruppe: Scheduler lädt Modell, Client führt Inferenz aus
    4. Ergebnisse zusammenführen in Original-Reihenfolge

Energiemessung: P_idle als GPU-Baseline. Für einen fairen Vergleich mit der
Baseline beginnt das Messfenster vor Router-Load und initialem Smart-Preload.
Damit sind auch die Startkosten des Router-Setups Teil der gemessenen Ausführung.
"""
...
from src.routing import LLMRouter
from src.routing.router import (
    build_router_few_shot_examples,
...
def _build_parallel_waves(execution_plan: list) -> list[list]:
    ...
    Bei Single-GPU: Jede Welle hat genau eine Gruppe (keine Parallelität).
    """
    waves: list[list] = []
...
def execute_router_measurement(
    config: dict,
    energy_monitor,
    benchmark_dataset,
    batch_size: int = 32,
    ...
    """Führt das Router-Experiment mit Scheduler durch.
    ...
    router_runtime = get_router_runtime_config(config)
    logger.info(
        f"Router-Budget: {router_runtime.budget_gb:.2f}GB "
        f"-> gpu_memory_utilization {router_runtime.gpu_memory_utilization:.4f}"
    )
    logger.info(
        "Router-Runtime: max_model_len=%d, enforce_eager=%s",
        router_runtime.max_model_len,
        router_runtime.enforce_eager,
    )
    ...
    router = LLMRouter(
        ...
        models=loadable_dicts,
        gpu_memory_utilization=router_runtime.gpu_memory_utilization,
        max_model_len=router_runtime.max_model_len,
        max_output_tokens=router_config.get("max_output_tokens", 10),
        dtype=config.get("inference", {}).get("vllm", {}).get("dtype", "auto"),
        enforce_eager=router_runtime.enforce_eager,
    ...
    scheduler = ModelScheduler(
        config, server_manager, nvlink_available=nvlink_available
    )
    ...
    logger.info(">>> Energiemessung START (vor Router-Load und initialem Preload)")
    energy_monitor.start_measurement()

    # 3. Smart Preload (Router-Load überlappt mit GPU-1-Modellen)
    preloaded = scheduler.preload_with_router_overlap(router.load)
    ...
    for batch in tqdm(loader, desc="Router+Scheduler", total=num_batches):
        batch_prompts = batch["prompts"]
        batch_references = batch["references"]
        batch_metadata = batch["metadata"]

        # 5a. Router: Batch klassifizieren
        routing_results = router.route_batch(batch_prompts)
        ...
        for rr in routing_results:
            ...
            if rr.routing_failed:
                routing_failures += 1

        # 5c. Execution Plan erstellen
        ...
        # 5d. Parallele Inferenz in Wellen
        waves = _build_parallel_waves(execution_plan)

        for wave in waves:
            # Alle Modelle in der Welle müssen RUNNING sein
            for group in wave:
            ...
            if len(wave) == 1:
                ...
                with ThreadPoolExecutor(max_workers=len(wave)) as executor:
                    ...
                    for group in wave:
                        ...
                        futures[future] = (group, group.indices)

                    for future in as_completed(futures):
                        group, global_indices = futures[future]
        ...
        # 5e. Ergebnisse zusammenführen (Original-Reihenfolge)
        for idx in range(len(batch_prompts)):
            ...
            result_row = {
                ...
                "routing_failed": rr.routing_failed,
                "routing_error": rr.error_message,
    ...
    if routing_failures > 0:
        logger.warning(f"Routing-Fehler: {routing_failures}")
    ...
    measurement_metrics = {
        ...
        "measurement_routing_failure_rate": (
            routing_failures / num_results if num_results > 0 else 0
        ),
...
def _build_experiment_config(
    ...
    return {
        ...
        "router": {
            ...
            "vram_gb": router_cfg.get("vram_gb"),
            "max_output_tokens": router_cfg.get("max_output_tokens"),
            "system_prompt": build_router_system_prompt(router_models),
            "few_shot_examples": build_router_few_shot_examples(router_models),
        },
        "models": [
...
def main():
    """Hauptfunktion für Router-Experiment."""
    import argparse

    available_benchmarks = list_benchmarks()

    parser = argparse.ArgumentParser(
        description="Router-Experiment: Multi-Modell mit Scheduler"
    )
    parser.add_argument(
        "--profile",
        "-p",
    ...
    logger.info("=" * 60)
    logger.info("LLM-Routing Energiemessung - Router + Scheduler")
    logger.info(f"Profil: {config['_profile']}")
    logger.info(f"Router: {config['router']['model']}")
    ...
    # P_idle als einheitliche Baseline
    try:
        with create_energy_monitor(
            ...
            # Idle-Power messen (P_idle Baseline)
            ...
            measurement = execute_router_measurement(
                config=config,
    ...
    if measurement is not None or failed:
        ...
        if not failed:
            ...
            logger.info("\nRouter-Experiment abgeschlossen!")
        else:
            logger.info(
